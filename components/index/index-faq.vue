		...
		<div class="container">
			...
			<div class="faq-wrap">
				<div v-for="(item, idx) in faq" :key="idx" class="item">
					<div class="question fd-r jc-b" @click="item.active = !item.active">
						<div class="fd-r ai-c p-L t-black t-b">
							<span class="decor mr-2">?</span>
							<span class="question-text">{{ item.question }}</span>
						</div>
						<button :class="{rotate: item.active}">
							<img src="@/assets/images/faq/arrow.svg" alt="Развернуть/Свернуть ответ" />
						</button>
					</div>
					<transition-expand>
						<div v-if="item.active" ref="answer" class="answer p-L t-gray">{{ item.answer }}</div>
					</transition-expand>
				</div>
			</div>
		</div>
