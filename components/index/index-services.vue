<template>
	<div class="services bg-anim" id="services">
      ...
      <span class="p-L t-gray-light mt-2">{{$t('services.sub')}}</span>
			...
			<div class="row fd-r jc-c ai-s fw-w">
				<template v-if="target.category == 'marketing'">
					...
					>
						<h3 class="p-L t-b t-red">{{ item.name }}</h3>
						<p class="p t-gray-light mt-3" v-html="item.text ? item.text.substring(0, 67) + '..' : ''"></p>
					</div>
				</template>
			...
			<div class="info fd-r ai-b jc-b">
				...
				<app-anchor :text="$t('services.button')" v-smooth-scroll red link="#contact" />
			</div>
		</div>
	</div>
</template>
...
.services {
	...
	background: url("@/assets/images/services/bg.svg") no-repeat right top;
	.container {
		.toggler {
			position: relative;
			width: 100%;
